...
export default function ModalContainerLender({
  id_trb,
  lender,
}: {
  id_trb: string;
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.textModalcen}>
        Nomor : {trbData?.id_ub}/DPI/PM/{mmyy}
      </Text>
      ...
      <View style={styles.column1}>
        <Text>1.</Text>
        <View>
          <Text>Nama : {lender.business}</Text>
          <Text>Alamat : {lender.address}</Text>
          <Text>No ktp : {lender.nik}</Text>
          <Text>Email : {lender.email}</Text>
          <Text>No hp : 0{lender.phone}</Text>
          <Text>Nomor ID Pendana : {lender.id_ul}</Text>
          <Text>-untuk selanjutnya akan disebut : PENDANA</Text>
        </View>
      </View>
      <View style={styles.column1}>
        <Text>2.</Text>
        <View>
          <Text>Nama : {trbData?.name}</Text>
          <Text>Alamat : {trbData?.address}</Text>
          <Text>No ktp : {trbData?.nik}</Text>
          <Text>Email : {trbData?.email}</Text>
          <Text>No hp : 0{trbData?.phone}</Text>
          <Text>Nomor ID Peminjam : {trbData?.id_ub}</Text>
      ...
      <Text style={styles.textModaljus}>
        <Text style={styles.textModalb}>“Biaya Layanan”</Text>
        adalah biaya atas penggunaan dan pemanfaatan layanan Situs/Aplikasi
        dalam pelaksanaan transaksi pembelian barang/jasa yang dilakukan oleh
        Pengguna pada Situs/Aplikasi{" "}
      </Text>
      <Text style={styles.textModaljus}>
      ...
      <Text style={styles.textModaljus}>
        g. Biaya Layanan atas Pinjaman akan dikenakan kepada Peminjam sesuai
        durasi jangka waktu yang disetujui oleh Peminjam, antara lain :
      </Text>
      <Text style={styles.textModalls}>
        1. untuk jangka waktu 61 (enam puluh satu) hari biaya layanan yang
        dikenakan sebesar 10 % (sepuluh persen) dari plafon pinjaman;
      </Text>
      <Text style={styles.textModalls}>
        2. untuk jangka waktu 75 (tujuh puluh lima) hari biaya layanan yang
        dikenakan sebesar 12,5% (dua belas koma lima persen) dari plafon
        pinjaman;
      </Text>
      <Text style={styles.textModalls}>
        3. untuk jangka waktu 90 (sembilan puluh) hari biaya layanan yang
        dikenakan sebesar 15% (lima belas persen) dari plafon pinjaman;
      </Text>
      <Text style={styles.textModaljus}>
        h. Tenor Pinjaman dinyatakan dalam Hari Kalender;
      </Text>
      <Text style={styles.textModaljus}>
        i. Jenis pinjaman adalah Pinjaman Tanpa Jaminan;
      </Text>
      <Text style={styles.textModaljus}>
        j. Jangka waktu pinjaman akan disesuaikan dengan batas waktu yang telah
      ...
      <Text style={styles.textModaljus}>
        k. Pelunasan atas Pinjaman dilakukan oleh Peminjam dengan cara
      ...
      <Text style={styles.textModaljus}>
        l. Peminjam akan melakukan pembayaran atas fasilitas pinjaman Pribadi
      ...
      <Text style={styles.textModaljus}>
        m. Pendana tidak bertanggung jawab bilamana Peminjam telah lalai atau
        salah didalam melakukan pembayaran sepanjang tidak sesuai dengan yang
        ditentukan oleh Penyelenggara ;{" "}
      </Text>
      <Text style={styles.textModaljus}>
        n. Pendana tidak terlibat baik secara langsung dan atau tidak langsung
        dengan jaringan terorisme, Organisasi Kejahatan, Sindikat Pencucian
        Uang, dan hal – hal lain yang melawan hukum;
      </Text>
      <Text style={styles.textModaljus}>
        o. Pendana tidak dalam sengketa atau tersangkut apapun juga atau yang
      ...
      <Text style={styles.textModaljus}>
        p. Peminjam tidak dapat mengalihkan setiap hak-hak dan/atau
      ...
      <Text style={styles.textModaljus}>
        q. Peminjam wajib melakukan tanda tangan elektronik berdasarkan
      ...
      {/* <Text style={styles.textModaljus}>
        ...
        {amount && toRupiah(amount || 0, { floatingPoint: 0 })},- (
        {terbilang(amount || 0)} rupiah) Jumlah Pinjaman tersebut diatas akan
        dikurangi dengan biaya layanan sebesar{" "}
        {toRupiah(
          trbData?.tenor == 61
            ? amount || 0 * 0.1
            : trbData?.tenor == 75
        ...
        {toRupiah(
          trbData?.tenor == 61
            ? amount || 0 - amount || 0 * 0.1
            : trbData?.tenor == 75
              ? amount || 0 - amount || 0 * 0.125
              : amount || 0 - amount || 0 * 0.15,
          { floatingPoint: 0 }
        )}
        ,- (
        {terbilang(
      ...
      {/* <Text style={styles.textModaljus}>
        ...
        {amount && toRupiah(amount || 0, { floatingPoint: 0 })},- (
        {terbilang(amount || 0)} rupiah) akan dikenakan Suku Bunga Pinjaman
        sebesar{" "}
        {trbData?.tenor == 61
          ? "0.15%"
          : trbData?.tenor == 75
      ...
      <Text style={styles.textModaljus}>
        ...
        : {amount && toRupiah(amount || 0 + bunga, { floatingPoint: 0 })},- (
        {terbilang(amount || 0 + bunga)} rupiah)
      </Text>
      {/* <Text style={styles.textModaljus}>
        a. Untuk jangka waktu {trbData?.tenor} (
        {terbilang(trbData?.tenor ? trbData?.tenor : 0)}) hari, dilakukan
        pembayaran dengan mengangsur sebanyak {trbData?.tenor == 61 ? "2" : "3"}{" "}
      ...
      {/* {trbData?.tenor == 61 ? (
        <>
          <Text style={styles.textModalls}>
            • Angsuran I (Pokok dan Bunga) : 30 (tiga puluh) hari{" "}
            {repayone && toRupiah(repayone || 0, { floatingPoint: 0 })},- (
            {terbilang(repayone || 0)} rupiah)
          </Text>
          <Text style={styles.textModalls}>
            • Angsuran II (Pokok dan Bunga) : 31 (tiga puluh satu) hari
            {repaytwo && toRupiah(repaytwo || 0, { floatingPoint: 0 })},- (
            {terbilang(repaytwo || 0)} rupiah)
          </Text>
        </>
      ) : trbData?.tenor == 75 ? (
        <>
          ...
          <Text style={styles.textModalls}>
            ...
            {repaythree && toRupiah(repaythree || 0, { floatingPoint: 0 })},- (
            {terbilang(repaythree || 0)} rupiah)
          </Text>
        </>
      ) : (
        <>
      ...
      {/* <Text style={styles.textModaljus}>
        ...
        {terbilang(trbData?.tenor ? trbData?.tenor : 0)} ({trbData?.tenor}) hari
        terhitung sejak tanggal {compDate} ({bulanTerbilang(hari, bulan, tahun)}
        ) dan berakhir pada tanggal {future} (
        {bulanTerbilang(hfuture, bfuture, tfuture)}). Jangka waktu Pinjaman
        dilakukan dengan cara mengangsur dan jadwal Angsuran akan tertera pada
        Platform Aplikasi Doeku masing-masing Peminjam.
      </Text> */}
      ...
      {/* <Text style={styles.textModaljus}>
        ...
        dengan batas waktu pinjaman maksimal {trbData?.tenor} (
        {terbilang(trbData?.tenor ? trbData?.tenor : 0)}) hari yang dihitung
        dari jumlah pinjaman dan pembayaran jumlah pinjaman tersebut dianggap
        telah lunas dibayar dengan bukti pembayaran yang telah disetujui oleh
        Pendana.
      </Text> */}
      ...
      <Text style={styles.textModalcen}>Nama : {trbData?.id_ub}</Text>
      <Text style={styles.textModalcen}>Alamat : {trbData?.address}</Text>
      <Text style={styles.textModalcen}>E-mail : {trbData?.email}</Text>
      <Text style={styles.textModalcen}>Telepon : 0{trbData?.phone}</Text>
      <Text style={styles.textModaljus}>
      ...
      <View style={styles.columnttds}>
        ...
        <View style={styles.columnttd}>
          ...
          <View style={styles.columnttdDet}>
            {/* <Image
              source={{
                uri: `${process.env.EXPO_PUBLIC_SERVICE_J1}/sign/borrower/${trbData?.sign}`,
              }}
              style={styles.ttdImage}
            /> */}
            <View>
              ...
              <Text style={styles.textModaljus}>{trbData?.name} </Text>
              <Text style={styles.textModaljus}>
                {/* {trbData?.requestdate} WITA */}
              </Text>
            </View>
          </View>
      ...
      {/* <Text style={styles.textModaljus}>
        Jangka waktu {trbData?.tenor} (
        {terbilang(trbData?.tenor ? trbData?.tenor : 0)}) hari dikenakan bunga
        sebesar{" "}
        {trbData?.tenor == 61
          ? "0.15%"
          : trbData?.tenor == 75
      ...
      {trbData?.tenor == 61 ? (
        <>
          <Text style={styles.textModalls}>
            • Angsuran I : ({trbData?.tenor} hari x 0,15 %) = 9,15 % (bunga)
            dengan Rincian : (jumlah pinjaman x {trbData?.tenor} hari x 0.15%)/2
            + (jumlah pinjaman/2) ={" "}
            {toRupiah((amount || 0 * 0.0015 * trbData?.tenor) / 2)} +{" "}
            {amount && toRupiah(amount || 0 / 2)} =
            {repayone && toRupiah(repayone || 0)}
          </Text>
          <Text style={styles.textModalls}>
            • Angsuran II : ({trbData?.tenor} hari x 0,15 %) = 9,15 % (bunga)
            dengan Rincian : (jumlah pinjaman x {trbData?.tenor} hari x 0.15%)/2
            + (jumlah pinjaman/2) ={" "}
            {toRupiah((amount || 0 * 0.0015 * trbData?.tenor) / 2)} +{" "}
            {amount && toRupiah(amount || 0 / 2)} =
            {repaytwo && toRupiah(repaytwo || 0)}
          </Text>
        </>
      ) : trbData?.tenor == 75 ? (
        <>
          {" "}
          <Text style={styles.textModalls}>
            • Angsuran I : ({trbData?.tenor} hari x 0,13 %) = 9,75 % (bunga)
            dengan Rincian : (jumlah pinjaman x {trbData?.tenor} hari x 0.13%)/3
            + (jumlah pinjaman/3) ={" "}
            {toRupiah((amount || 0 * 0.0013 * trbData?.tenor) / 3)} +{" "}
            {amount && toRupiah(amount || 0 / 3)} =
            {repayone && toRupiah(repayone || 0)}
          </Text>
          <Text style={styles.textModalls}>
            • Angsuran II : ({trbData?.tenor} hari x 0,13 %) 3 = 9,75 % (bunga)
            dengan Rincian : (jumlah pinjaman x {trbData?.tenor} hari x 0.13%)/3
            + (jumlah pinjaman/3) ={" "}
            {toRupiah((amount || 0 * 0.0013 * trbData?.tenor) / 3)} +{" "}
            {amount && toRupiah(amount || 0 / 3)} =
          ...
          <Text style={styles.textModalls}>
            • Angsuran III : ({trbData?.tenor} hari x 0,13 %) 3 = 9,75 % (bunga)
            dengan Rincian : (jumlah pinjaman x {trbData?.tenor} hari x 0.13%)/3
            + (jumlah pinjaman/3) ={" "}
            {toRupiah((amount || 0 * 0.0013 * trbData?.tenor) / 3)} +{" "}
            {amount && toRupiah(amount || 0 / 3)} =
            {repaythree && toRupiah(repaythree || 0)}
          </Text>
        </>
      ) : (
        <>
          <Text style={styles.textModalls}>
            • Angsuran I : ({trbData?.tenor} hari x 0,1 %) = 9 % (bunga) dengan
            Rincian : (jumlah pinjaman x {trbData?.tenor} hari x 0.1%)/3 +
            (jumlah pinjaman/3) ={" "}
            {toRupiah((amount || 0 * 0.001 * trbData?.tenor) / 3)} +{" "}
            {amount && toRupiah(amount || 0 / 3)} =
            {repayone && toRupiah(repayone || 0)}
          </Text>
          <Text style={styles.textModalls}>
            • Angsuran II : ({trbData?.tenor} hari x 0,1 %) = 9 % (bunga) dengan
            Rincian : (jumlah pinjaman x {trbData?.tenor} hari x 0.1%)/3 +
            (jumlah pinjaman/3) ={" "}
            {toRupiah((amount || 0 * 0.001 * trbData?.tenor) / 3)} +{" "}
            {amount && toRupiah(amount || 0 / 3)} =
          ...
          <Text style={styles.textModalls}>
            • Angsuran III : ({trbData?.tenor} hari x 0,1 %) = 9 % (bunga)
            dengan Rincian : (jumlah pinjaman x {trbData?.tenor} hari x 0.1%)/3
            + (jumlah pinjaman/3) ={" "}
            {toRupiah((amount || 0 * 0.001 * trbData?.tenor) / 3)} +{" "}
            {amount && toRupiah(amount || 0 / 3)} =
            {repaytwo && toRupiah(repaytwo || 0)}
      ...
      {/* <Text style={styles.textModaljus}>
        Jangka waktu pinjaman terhitung sejak Peminjam telah mendapatkan
        pinjaman dari Pendana sampai dengan jangka waktu yang di sepakati.
        Jangka waktu pinjaman selama Sembilan Puluh ({trbData?.tenor}) hari
        terhitung sejak tanggal {compDate} ({bulanTerbilang(hari, bulan, tahun)}
        ) dan berakhir pada tanggal
        {future} ({bulanTerbilang(hfuture, bfuture, tfuture)}). Jangka waktu
        Pinjaman dilakukan secara mengangsur sedangkan jangka waktu Pembayaran
        Angsuran akan tertera pada Platform Aplikasi Doeku masing- masing
        Peminjam.
      </Text> */}
      <Text style={styles.textModalb}> Denda Keterlambatan</Text>
      {/* <Text style={styles.textModaljus}>
        ...
        dengan batas maksimal jangka waktu pembayaran {trbData?.tenor} (
        {terbilang(trbData?.tenor ? trbData?.tenor : 0)}) hari. Apabila lewat
        dari jangka waktu {trbData?.tenor} (
        {terbilang(trbData?.tenor ? trbData?.tenor : 0)}) hari Peminjam tidak
        melunasi Jumlah Pinjaman, Suku Bunga dan Denda Bunga, maka Peminjam di
        anggap Wansprestasi sesuai dengan Pasal 7 di atas, maka Pihak Pendana
        dan Penyelenggara DOEKU dapat mengajukan Blacklist dan atau BI Checking
        ke Lembaga Otoritas Jasa Keuangan (OJK).
      </Text> */}
    </View>
  );
}
